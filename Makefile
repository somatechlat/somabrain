...
install: venv
	# Install nested memory backend first (editable), then this project
	@if [ -d somafractalmemory ]; then $(PIP) install -e somafractalmemory; fi
	$(PIP) install -e .
...
docker-run:
	docker run --rm -p 9696:9696 -e SOMABRAIN_PORT=9696 -e SOMABRAIN_MEMORY_MODE=local $(IMAGE):$(TAG)

docker-build-prod:
	docker build -t $(IMAGE):prod .

docker-run-prod:
	docker run --rm -p 9696:9696 -e SOMABRAIN_PORT=9696 -e SOMABRAIN_MEMORY_MODE=local $(IMAGE):prod
