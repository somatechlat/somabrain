...
export REDIS_HOST_PORT REDIS_CONTAINER_PORT \
  ...
  SOMABRAIN_PORT SOMABRAIN_HOST_PORT SOMABRAIN_INTEGRATOR_HEALTH_PORT
...
INTEGRATOR_PORT=${SOMABRAIN_INTEGRATOR_HEALTH_PORT:-9015}

for i in {1..30}; do
  if curl -fsS "http://${HOST}:${PORT}/health" >/dev/null 2>&1; then
    if curl -fsS "http://localhost:${INTEGRATOR_PORT}/health" >/dev/null 2>&1; then
