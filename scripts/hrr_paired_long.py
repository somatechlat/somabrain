                ...
                for mode, layer in [("robust", qlayer), ("pure", player)]:
                    ...
                    for t in range(trials):
                        ...
                        try:
                            ...
                            cos = layer.cosine(x, x_rec) if hasattr(layer, "cosine") else float((x * x_rec).sum())
                        except Exception:
                            cos = float("nan")
                        cosines.append(cos)
                    ...
                    fail_rate = sum(1 for c in cosines if not (c == c) or c < 0.9) / len(cosines)
                    rows.append({
                        "dim": dim,
                        "dtype": dtype,
                        "eps": eps,
                    ...
                    print(f"dim={dim} dtype={dtype} eps={eps:.1e} mode={mode} med={med:.6f} fail_rate={fail_rate:.3f}")
    # write CSV
