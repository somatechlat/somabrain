...
class SomaBrainFoundation:
    ...
    def remember(self, content: str, metadata: Optional[Dict] = None) -> bool:
        """
    ...
    def recall(self, query: str, limit: int = 5) -> List[Dict]:
        ...
        try:
            ...
            results = []
            for node, similarity in fractal_results:
                results.append({
                    'content': str(node.memory_trace),
                    'similarity': float(similarity),
                    'metadata': {}
                })

            # Enhance with working memory if available
            if self.attention and results:
                ...
                for result in results:
                    try:
                        ...
                        context_memories = self.working_memory.items("default", limit=5)
                        context_dicts = [{'content': item['content'] if 'content' in item else str(item), 'embedding': None} for item in context_memories]

                        salience = self.attention.calculate_fractal_salience(
                            result['content'], content_embedding, context_dicts
                        )

                        # Ensure salience is a valid number
                        if salience is not None and isinstance(salience, (int, float)):
                            attention_weights.append(float(salience))
                        else:
                            attention_weights.append(0.0)
                    except Exception as e:
                        print(f"⚠️ Attention calculation failed for result: {e}")
                        attention_weights.append(0.0)
        ...
        try:
            ...
            if self.attention and context:
                ...
                for memory in context:
                    ...
                    context_memories = self.working_memory.items("default", limit=3)
                    context_dicts = [{'content': item['content'], 'embedding': None} for item in context_memories]
    ...
    def get_status(self) -> Dict:
        ...
        """
        status = {
            'fractal_memory': 'working' if self.fractal_memory else 'not available',
            'embeddings': 'working' if self.embedder else 'not available',
            'working_memory': 'working' if self.working_memory else 'not available',
            'attention': 'working' if self.attention else 'not available',
