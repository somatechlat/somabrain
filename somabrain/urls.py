...
def health_view(request):
    ...
    def timed_check(name, check_func):
        ...
        try:
            ...
            return {"name": name, "status": "healthy" if result else "degraded", 
                    "response_time_ms": elapsed, "details": result}
        except Exception as e:
            elapsed = int((time.time() - start) * 1000)
            return {"name": name, "status": "unhealthy", 
                    "response_time_ms": elapsed, "error": str(e)[:100]}
    ...
    def check_pg():
        ...
        with connection.cursor() as cursor:
            ...
            return {"version": version.split(",")[0] if version else "unknown", 
    ...
    def check_schema_registry():
        ...
        with httpx.Client(timeout=3) as c:
            ...
            return {"healthy": r.status_code == 200, "subjects": len(r.json()) if r.status_code == 200 else 0}
    health["infrastructure"]["schema_registry"] = timed_check("Schema Registry", check_schema_registry)
    ...
    health["degraded_count"] = sum(1 for s in all_svc if s.get("status") in ("degraded", "unavailable"))
