"""Graph operations mixin for MemoryClient.
...
class GraphOpsMixin:
    ...
    def _queue_to_outbox(self, topic: str, payload: Dict[str, Any], tenant: str) -> None:
        ...
        try:
            ...
            logger.warning("Failed to queue to outbox", error=str(exc))
    ...
    def create_link(
        ...
        with tracer.start_as_current_span("sb_graph_link_create") as span:
            ...
            if self._http is None or self._http.client is None:
                logger.warning("GraphOpsMixin: transport not available")
                SB_GRAPH_LINK_TOTAL.labels(
                    tenant=tenant, link_type=link_type, status="no_transport"
                ).inc()
            ...
            headers = self._graph_headers(tenant)
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    SB_GRAPH_LINK_LATENCY.labels(
                        tenant=tenant, link_type=link_type
                    ).observe(duration)
                    return True
                else:
                    SB_GRAPH_LINK_TOTAL.labels(
                        tenant=tenant, link_type=link_type, status="http_error"
                    ).inc()
                    SB_GRAPH_LINK_LATENCY.labels(
                        tenant=tenant, link_type=link_type
                    ).observe(duration)
                ...
                duration = time.perf_counter() - start_time
                SB_GRAPH_LINK_TOTAL.labels(
                    tenant=tenant, link_type=link_type, status="exception"
                ).inc()
                SB_GRAPH_LINK_LATENCY.labels(
                    tenant=tenant, link_type=link_type
                ).observe(duration)
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    SB_GRAPH_NEIGHBORS_TOTAL.labels(
                        tenant=tenant, status="http_error"
                    ).inc()
                    SB_GRAPH_NEIGHBORS_LATENCY.labels(tenant=tenant).observe(duration)
                    logger.warning(
                ...
                duration = time.perf_counter() - start_time
                SB_GRAPH_NEIGHBORS_TOTAL.labels(tenant=tenant, status="exception").inc()
                SB_GRAPH_NEIGHBORS_LATENCY.labels(tenant=tenant).observe(duration)
                span.record_exception(exc)
    ...
    def find_path(
        ...
        with tracer.start_as_current_span("sb_graph_path_query") as span:
            ...
            if self._http is None or self._http.client is None:
                logger.warning("GraphOpsMixin: transport not available for find_path")
                SB_GRAPH_PATH_TOTAL.labels(
                    tenant=tenant, status="no_transport", found="false"
                ).inc()
                return []
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if data.get("found"):
                        ...
                        SB_GRAPH_PATH_TOTAL.labels(
                            tenant=tenant, status="success", found="true"
                        ).inc()
                        SB_GRAPH_PATH_LATENCY.labels(tenant=tenant).observe(duration)
                        return path
                    span.set_attribute("path_found", False)
                    SB_GRAPH_PATH_TOTAL.labels(
                        tenant=tenant, status="success", found="false"
                    ).inc()
                    SB_GRAPH_PATH_LATENCY.labels(tenant=tenant).observe(duration)
                    return []
                else:
                    SB_GRAPH_PATH_TOTAL.labels(
                        tenant=tenant, status="http_error", found="false"
                    ).inc()
                    SB_GRAPH_PATH_LATENCY.labels(tenant=tenant).observe(duration)
                    logger.warning(
                ...
                duration = time.perf_counter() - start_time
                SB_GRAPH_PATH_TOTAL.labels(
                    tenant=tenant, status="exception", found="false"
                ).inc()
                SB_GRAPH_PATH_LATENCY.labels(tenant=tenant).observe(duration)
                span.record_exception(exc)
