...
class AdaptationEngine:
    ...
    def __init__(
        ...
        lr = learning_rate if learning_rate is not None else (
            getattr(settings, "adaptation_learning_rate", 0.05) if settings else 0.05
        )
        self._lr = lr
        self._base_lr = lr
        self._history: list = []
        self._max_history = int(max_history if max_history is not None else (
    ...
    def _init_constraints(self, constraints: AdaptationConstraints | dict | None) -> AdaptationConstraints:
        if isinstance(constraints, AdaptationConstraints):
            ...
            attr_map = {"alpha": ("alpha_min", "alpha_max"), "gamma": ("gamma_min", "gamma_max"),
                        "lambda_": ("lambda_min", "lambda_max"), "mu": ("mu_min", "mu_max"), "nu": ("nu_min", "nu_max")}
            for key, (lower, upper) in constraints.items():
                if key in attr_map:
                    constraint_bounds = replace(constraint_bounds, **{attr_map[key][0]: float(lower), attr_map[key][1]: float(upper)})
    ...
    def _build_constraints_dict(self, bounds: AdaptationConstraints) -> dict:
        return {"alpha": (bounds.alpha_min, bounds.alpha_max), "gamma": (bounds.gamma_min, bounds.gamma_max),
                "lambda_": (bounds.lambda_min, bounds.lambda_max), "mu": (bounds.mu_min, bounds.mu_max), "nu": (bounds.nu_min, bounds.nu_max)}
    ...
    def _maybe_load_state(self) -> None:
        if (self._redis and self._tenant_id and settings and
            str(getattr(settings, "enable_learning_state_persistence", "")).strip().lower() in {"1", "true", "yes", "on"}):
            self._load_state()
    ...
    def reset(self, retrieval_defaults: Optional[RetrievalWeights] = None, utility_defaults: Optional[UtilityWeights] = None,
              base_lr: Optional[float] = None, clear_history: bool = True) -> None:
        if retrieval_defaults is not None:
            self._retrieval.alpha, self._retrieval.beta = float(retrieval_defaults.alpha), float(retrieval_defaults.beta)
            self._retrieval.gamma, self._retrieval.tau = float(retrieval_defaults.gamma), float(retrieval_defaults.tau)
        else:
            try:
                ...
                self._retrieval.alpha, self._retrieval.beta, self._retrieval.gamma, self._retrieval.tau = 1.0, 0.2, 0.1, 0.7
    ...
    def _state(self) -> dict:
        return {"tenant_id": self._tenant_id, "retrieval": {"alpha": self._retrieval.alpha, "beta": self._retrieval.beta,
                "gamma": self._retrieval.gamma, "tau": self._retrieval.tau}, "utility": {"lambda_": self._utility.lambda_,
                "mu": self._utility.mu, "nu": self._utility.nu}, "feedback_count": getattr(self, "_feedback_count", 0),
    ...
    def _apply_weight_updates(self, semantic_signal: float, utility_signal: float) -> None:
        self._retrieval.alpha = self._constrain("alpha", self._retrieval.alpha + self._lr * self._gains.alpha * semantic_signal)
        self._retrieval.gamma = self._constrain("gamma", self._retrieval.gamma + self._lr * self._gains.gamma * semantic_signal)
        self._utility.lambda_ = self._constrain("lambda_", self._utility.lambda_ + self._lr * self._gains.lambda_ * utility_signal)
        self._utility.mu = self._constrain("mu", self._utility.mu + self._lr * self._gains.mu * utility_signal)
        self._utility.nu = self._constrain("nu", self._utility.nu + self._lr * self._gains.nu * utility_signal)
        self._utility.clamp(lambda_bounds=self._constraints["lambda_"], mu_bounds=self._constraints["mu"], nu_bounds=self._constraints["nu"])

    def _apply_tau_and_entropy(self) -> None:
        if not hasattr(self, "_tenant_override") or getattr(self, "_tenant_override_id", None) != self._tenant_id:
            self._tenant_override = _get_tenant_override(self._tenant_id)
            self._tenant_override_id = self._tenant_id
        new_tau, was_annealed = apply_tau_annealing(self._retrieval.tau, self._tenant_id, getattr(self, "_feedback_count", 0), self._tenant_override)
        self._retrieval.tau = new_tau
        self._retrieval.tau = apply_tau_decay(self._retrieval.tau, self._tenant_id, self._tenant_override, skip_if_annealed=True, was_annealed=was_annealed)
        check_entropy_cap(self._retrieval.alpha, self._retrieval.beta, self._retrieval.gamma, self._retrieval.tau, self._tenant_id)
    ...
    def rollback(self) -> bool:
        ...
        self._utility.lambda_, self._utility.mu, self._utility.nu = prev_utility["lambda_"], prev_utility["mu"], prev_utility["nu"]
        return True

    def _update_metrics(self) -> None:
        try:
            from somabrain import metrics as _metrics
            _metrics.update_learning_retrieval_weights(tenant_id=self._tenant_id, alpha=self._retrieval.alpha, beta=self._retrieval.beta, gamma=self._retrieval.gamma, tau=self._retrieval.tau)
    ...
    def _load_state(self) -> None:
        ...
        if "retrieval" in state:
            ...
            self._retrieval.gamma, self._retrieval.tau = r.get("gamma", self._retrieval.gamma), r.get("tau", self._retrieval.tau)
        if "utility" in state:
            u = state["utility"]
            self._utility.lambda_, self._utility.mu, self._utility.nu = u.get("lambda_", self._utility.lambda_), u.get("mu", self._utility.mu), u.get("nu", self._utility.nu)
    ...
    def optimize_hyperparameters(self, search_space: dict, n_trials: int = 10) -> dict:
        lr_opts, tau_opts = list(search_space.get("learning_rate", [self._base_lr])), list(search_space.get("tau", [self._retrieval.tau]))
