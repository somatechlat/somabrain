...
class TenantManager:
    ...
    async def resolve_tenant_from_request(self, request: HttpRequest) -> str:
        ...
        if not getattr(settings, "SOMABRAIN_ALLOW_ANONYMOUS_TENANTS", False):
            raise HttpError(
                401,
                "Anonymous tenant access is disabled. Configure a default tenant or enable SOMABRAIN_ALLOW_ANONYMOUS_TENANTS=1 explicitly.",
            )
...
async def get_tenant_manager() -> TenantManager:
    ...
    if _tenant_manager is None:
        ...
        redis_url = getattr(settings, "SOMABRAIN_REDIS_URL", None)
