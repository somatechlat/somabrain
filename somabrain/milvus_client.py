    ...
    *All* configuration values are read from the global ``settings`` instance –
    ...
    def __init__(self) -> None:
        ...
        try:
            ...
            try:
                M.MILVUS_CONNECT_LATENCY.observe(time.time() - start_time)
                M.MILVUS_CONNECT_SUCCESS.inc()
            except Exception:
                pass
        except MilvusException as exc:
            logger.error("Milvus connection failed (%s); client is unusable.", exc)
            try:
                M.MILVUS_CONNECT_FAILURE.inc()
            except Exception:
                pass
    ...
    def upsert_option(self, tenant_id: str, option_id: str, payload: bytes) -> None:
        ...
        try:
            ...
            try:
                M.MILVUS_UPSERT_LATENCY.observe(time.time() - start_time)
                M.MILVUS_UPSERT_SUCCESS.inc()
            except Exception:
                pass
        except Exception as e:
            try:
                M.MILVUS_UPSERT_FAILURE.inc()
            except Exception:
                pass
            raise e
    ...
    def search_similar(
        ...
        try:
            ...
            try:
                M.MILVUS_SEARCH_LATENCY.observe(time.time() - start_time)
                M.MILVUS_SEARCH_SUCCESS.inc()
            except Exception:
                pass
            return out
        except Exception as e:
            try:
                M.MILVUS_SEARCH_FAILURE.inc()
            except Exception:
                pass
            raise e
