...
def main():
    ...
    # Seed some episodics
    for i in range(3):
        r = client.post("/remember", json={"coord": None, "payload": {"task": f"mig task {i}", "importance": 1, "memory_type": "episodic"}})
    ...
    r = client.post("/migrate/import", json={"manifest": exp["manifest"], "memories": exp["memories"], "wm": exp.get("wm", [])})
    assert r.status_code == 200
    imp = r.json()
