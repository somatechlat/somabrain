...
def main():
    ...
    base = r.json()
    assert set(base.keys()) == {"dopamine", "serotonin", "noradrenaline", "acetylcholine"}

    # Set new values (some out of range to test clamping)
    r = client.post("/neuromodulators", json={"dopamine": 1.0, "serotonin": 0.9, "noradrenaline": 0.2, "acetylcholine": 0.05})
    assert r.status_code == 200
    nm = r.json()
