...
def main():
    ...
    for t in tasks:
        r = client.post("/remember", json={"coord": None, "payload": {"task": t, "importance": 2, "memory_type": "episodic"}})
        assert r.status_code == 200
    # Links: collect -> analyze (depends_on), analyze -> report (causes)
    r = client.post("/link", json={"from_key": tasks[1], "to_key": tasks[0], "type": "depends_on"})
    assert r.status_code == 200
    r = client.post("/link", json={"from_key": tasks[1], "to_key": tasks[2], "type": "causes"})
    assert r.status_code == 200
    ...
    assert any("collect data" in s for s in plan) or any("report results" in s for s in plan)
