...
def main():
    ...
    for t in tasks_cf:
        r = client.post("/remember", json={"coord": None, "payload": {"task": t, "universe": "cf:alt", "importance": 1, "memory_type": "episodic"}})
        assert r.status_code == 200
    r = client.post("/link", json={"from_key": tasks_cf[1], "to_key": tasks_cf[0], "type": "depends_on", "universe": "cf:alt"})
    assert r.status_code == 200
