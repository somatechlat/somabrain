...
def main():
    ...
    for t in tasks:
        r = client.post("/remember", json={"coord": None, "payload": {"task": t, "importance": 1, "memory_type": "episodic"}})
        assert r.status_code == 200

    r = client.post("/link", json={"from_key": tasks[0], "to_key": tasks[1], "type": "related"})
    assert r.status_code == 200
