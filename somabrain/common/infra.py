...
def check_redis(redis_url: Optional[str], timeout_s: float = 2.0) -> bool:
    ...
    fall back to ``settings.SOMABRAIN_REDIS_URL`` which is the single source of truth for
    the Redis endpoint across the codebase.
    """
    # Import lazily to avoid circular imports at module load time.
    # Directly use the imported ``settings`` singleton.
    url = (redis_url or getattr(settings, "SOMABRAIN_REDIS_URL", None) or "").strip()
    if not url:
        return False
    try:
        import redis
...
def check_postgres(dsn: Optional[str], timeout_s: float = 2.0) -> bool:
    ...
    value from ``settings.SOMABRAIN_POSTGRES_DSN`` which is the authoritative source for the
    database connection string.
    """
    # Directly use the imported ``settings`` singleton.
    dsn = (dsn or getattr(settings, "SOMABRAIN_POSTGRES_DSN", None) or "").strip()
    if not dsn:
        return False
    try:
        import psycopg
...
def assert_ready(
    *,
    ...
    """
    if str(getattr(settings, "require_infra", "1")).strip().lower() in {
        "0",
        "false",
        "no",
        "off",
    ...
    if require_redis:
        if not check_redis(
            getattr(settings, "SOMABRAIN_REDIS_URL", None),
            timeout_s=timeout_s,
        ):
            errors.append("Redis")
    if require_postgres:
        if not check_postgres(
            getattr(settings, "SOMABRAIN_POSTGRES_DSN", None),
            timeout_s=timeout_s,
        ):
            errors.append("Postgres")
    if require_opa:
        if not check_opa(
            getattr(settings, "SOMABRAIN_OPA_URL", None),
            timeout_s=timeout_s,
        ):
            errors.append("OPA")
    if errors:
