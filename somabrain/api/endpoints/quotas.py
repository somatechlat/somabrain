...
def get_tenant_quotas_real(tenant: Tenant) -> List[QuotaStatus]:
    ...
    users_used = TenantUser.objects.filter(tenant_id=tenant.id, is_active=True).count()
    ...
    api_keys_used = APIKey.objects.filter(tenant_id=tenant.id, is_active=True).count()
    ...
    for i in range(days):
        ...
        if resource == "users":
            ...
            used = TenantUser.objects.filter(
                tenant_id=tenant_id, created_at__lte=date, is_active=True
            ).count()
        else:
            used = 0
...
def get_tenants_near_limit(
    ...
    tenants = Tenant.objects.filter(
        status="active", subscription_tier__isnull=False
