...
def check_milvus() -> Dict[str, Any]:
    ...
    def _check():
        ...
        return {
            ...
            "collection_name": getattr(client.collection, "name", None) if client.collection else None,
...
def get_full_health(request):
    ...
    degraded = sum(1 for s in all_services if s["status"] in ("degraded", "unavailable", "not_configured"))
