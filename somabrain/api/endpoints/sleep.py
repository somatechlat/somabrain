...
def _process_sleep_transition(
    ...
    if not manager.can_transition(current_state_enum, target_state):
        raise HttpError(400, f"Invalid transition from {current_state_enum.value} to {target_state.value}")
    ...
    logger.info(f"Sleep transition ({mode}) for {tenant_id}: {current_state_enum.value} -> {target_state.value}")
...
def set_sleep_state(request: HttpRequest, payload: dict):
    ...
    return _process_sleep_transition(
        request,
        mode="util",
        target_state_str=payload.get("state", "active")
    )
