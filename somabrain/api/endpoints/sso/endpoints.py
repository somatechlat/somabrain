...
def create_identity_provider(
    request: AuthenticatedRequest, tenant_id: UUID, data: IdPCreate
):
...
def update_identity_provider(
    request: AuthenticatedRequest, tenant_id: UUID, idp_id: str, data: IdPUpdate
):
...
def delete_identity_provider(
    request: AuthenticatedRequest, tenant_id: UUID, idp_id: str
):
...
def update_sso_settings(
    request: AuthenticatedRequest, tenant_id: UUID, data: SSOSettings
):
    """Update SSO settings."""
    if not request.is_super_admin and str(request.tenant_id) != str(tenant_id):
        raise HttpError(403, "Access denied")
