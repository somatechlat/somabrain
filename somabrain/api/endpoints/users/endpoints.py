...
def list_tenant_users(
    request: AuthenticatedRequest, tenant_id: UUID, filters: UserFilters = Query(...)
):
...
def create_tenant_user(
    request: AuthenticatedRequest, tenant_id: UUID, data: UserCreate
):
        ...
        if data.roles:
            ...
            for role in roles:
                TenantUserRole.objects.create(
                    tenant_user=user, role=role, assigned_by_id=request.user_id
                )
        AuditLog.log(
            action="user.created",
...
def update_tenant_user(
    request: AuthenticatedRequest, tenant_id: UUID, user_id: UUID, data: UserUpdate
):
...
def assign_role(
    request: AuthenticatedRequest, tenant_id: UUID, user_id: UUID, data: RoleAssignment
):
...
@require_permission(Permission.USERS_ASSIGN_ROLE.value)
def remove_role(
    request: AuthenticatedRequest, tenant_id: UUID, user_id: UUID, role_id: UUID
):
...
def get_user_audit_log(request: AuthenticatedRequest, tenant_id: UUID, user_id: UUID):
    ...
    logs = AuditLog.objects.filter(
        resource_type="TenantUser", resource_id=str(user.id)
