...
def list_platform_providers(request: AuthenticatedRequest):
    ...
    """
    providers = IdentityProvider.objects.filter(tenant__isnull=True).order_by(
        "display_order", "name"
    )
...
def create_platform_provider(
    ...
    if data.is_default:
        IdentityProvider.objects.filter(tenant__isnull=True, is_default=True).update(
            is_default=False
        )
...
def update_platform_provider(
    request: AuthenticatedRequest, provider_id: UUID, data: IdentityProviderUpdate
):
    ...
    if update_data.get("is_default"):
        IdentityProvider.objects.filter(tenant__isnull=True, is_default=True).exclude(
            id=provider_id
...
def list_tenant_providers(request: AuthenticatedRequest, tenant_id: UUID):
    ...
    platform_providers = list(
        IdentityProvider.objects.filter(tenant__isnull=True, is_enabled=True).order_by(
            "display_order", "name"
        )
    )
...
def create_tenant_provider(
    request: AuthenticatedRequest, tenant_id: UUID, data: IdentityProviderCreate
):
    """
...
def test_provider_connection(
    ...
    try:
        ...
        if data.provider_id:
            ...
            return TestConnectionResult(success=False, message="No provider specified")
