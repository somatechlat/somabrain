...
class TenantRegistry:
    ...
    async def register_tenant(
        ...
        # Audit log
        await self._audit_log("tenant_registered", tenant_id, {
            "display_name": display_name,
            "tier": tier.value,
            "is_exempt": is_exempt,
            "created_by": created_by
    ...
    async def update_tenant_status(self, tenant_id: str, status: Union[TenantStatus, str]) -> bool:
        ...
        # Audit log
        await self._audit_log("tenant_status_updated", tenant_id, {
            "status": status.value
        })
    ...
    async def delete_tenant(self, tenant_id: str) -> bool:
        ...
        try:
            ...
            # Audit log
            await self._audit_log("tenant_deleted", normalized_id, {
    ...
    async def cleanup_expired_tenants(self) -> int:
        ...
        for tenant_id, metadata in list(self._tenant_cache.items()):
            if (metadata.expires_at and 
                metadata.expires_at <= current_time and 
                metadata.tier != TenantTier.SYSTEM):
