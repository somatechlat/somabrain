...
class TenantManager:
    ...
    async def list_tenants(
        ...
        for tenant in tenants:
            if status and tenant.status != (TenantStatus(status) if isinstance(status, str) else status):
                continue
    ...
    async def resolve_tenant_for_legacy_compatibility(self, tenant_id: Optional[str] = None) -> str:
