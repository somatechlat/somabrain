...
``settings.use_drift_monitor`` is ``True``.  In environments where the heavy‑
...
class DriftDetector:
    ...
    The detector is enabled when ``settings.use_drift_monitor`` is ``True``.
    ...
    persisted to ``settings.drift_store_path`` so that external tools (e.g.
    ...
    def __init__(self) -> None:
        self.enabled: bool = bool(getattr(settings, "use_drift_monitor", False))
