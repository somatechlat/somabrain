...
class LeaderMetrics:
    ...
    def record_dwell_violation(self, action: str) -> None:
        ...
        LEADER_DWELL_ENFORCEMENT.labels(
            tenant=self.tenant,
            action=action
        ).inc()
    ...
    def record_renewal_failure(self, reason: str) -> None:
        ...
        LEADER_RENEWAL_FAILURES.labels(
            tenant=self.tenant,
            reason=reason
        ).inc()
    ...
    def update_dwell_metrics(self, current_dwell_ms: float, min_dwell_ms: int, remaining_ms: float) -> None:
...
class LeaderMetricsCollector:
    ...
    def record_config_validation(self, tenant: str, valid: bool) -> None:
        ...
        LEADER_CONFIG_VALIDATION.labels(
            tenant=tenant,
...
def record_transition_complete(tenant: str, instance_id: str, reason: str, success: bool = True) -> None:
...
def record_constraint_check(tenant: str, instance_id: str, dwell_ms: float, min_dwell_ms: int, entropy: float, threshold: float, allowed: bool) -> None:
...
def calculate_health_score(tenant: str, instance_id: str) -> float:
    ...
    # Health factors
    tenure = time.time() - metrics.start_time
    renewal_success_rate = max(0, 1 - (metrics.renewal_failures / max(1, metrics.transition_count * 10)))
    ...
    health_score = (
        0.4 * renewal_success_rate +
        0.3 * dwell_compliance +
