...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(self.style.SUCCESS("✓ Constitution seeded successfully!"))
        ...
        if options["sign"]:
            ...
            if signature:
                self.stdout.write(self.style.SUCCESS("✓ Constitution signed!"))
                self.stdout.write(f"  Signature: {signature[:32]}...")
            else:
                self.stdout.write(
                    self.style.WARNING("⚠ Signing skipped (no private key configured)")
                )
        ...
        self.stdout.write("Binding Parties: Adrian (Human) ↔ AI Agents")
