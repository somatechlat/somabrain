...
async def run_retrieval_pipeline(
    ...
    *,
    ...
    - Fans out to retriever adapters by name
    - Dedupes by key/coord
    - Orders by score desc
    - Optionally persists a session record
    """
    ...
    try:
        ...
        try:
            logger.info(
                "retrieval_pipeline: error instantiating mem_client from runtime.mt_memory: %s",
                repr(_rt.mt_memory),
            )
        except Exception:
            pass
    ...
    try:
        logger.info(
            "Retrieval runtime state: embedder=%s mt_memory=%s mt_wm=%s mc_wm=%s mem_client=%s",
    ...
    pinned_exact: list[RetrievalCandidate] = []
            ...
            if _t0 is not None:
                try:
                    ...
                    try:
                        ...
                        pass
            try:
                M.RETRIEVAL_CANDIDATES.labels(**_metrics_ctx).observe(0)
            except Exception:
                try:
    ...
    try:
        _mx.RETRIEVAL_FUSION_APPLIED.labels(method=fusion_method).inc()
        _mx.RETRIEVAL_FUSION_SOURCES.observe(len(lists_by_retriever))
    except Exception:
        pass
    ...
    wmap = {
        ...
        "lexical": float(getattr(_rt.cfg, "retriever_weight_lexical", 1.0) or 1.0),
    }
    ...
    if pinned_exact:
        ...
        seen = set()
        pinned_unique: list[RetrievalCandidate] = []
        for c in pinned_exact:
            k = _kid(c)
            if k and k not in seen:
                seen.add(k)
                pinned_unique.append(c)
        rest: list[RetrievalCandidate] = []
        for c in out:
            k = _kid(c)
    ...
    # Optional persistence of session + links
    ...
    if bool(req.persist):
        try:
            ...
            if mem_backend is not None:
                ...
                # Ensure immediate visibility in the namespace client
                try:
                ...
                for c in out:
                    ...
                    is_session = False
                    if isinstance(payload_dict, dict) and payload_dict.get("retrieval"):
                        is_session = True
                    if isinstance(
                        key_for_coord, str
                    ) and key_for_coord.lower().startswith("recall session"):
                        is_session = True
                    ...
                    if not is_session:
                        ...
                        retr_info = (
                            payload_dict.get("retrieval")
                            if isinstance(payload_dict, dict)
                            else None
                        )
                        doc_keys: list[str] = []
                        if isinstance(retr_info, dict):
                            for entry in retr_info.get("candidates", []):
                                doc_key = (
                                    entry.get("key")
                                ...
                                if doc_key_str.lower().startswith("recall session"):
                                    continue
                ...
                if persist_records:
                    try:
                        retrieval_cache.store_candidates(
            ...
            try:
                ...
                M.RETRIEVAL_PERSIST.labels(status="failure").inc()
            except Exception:
                pass
