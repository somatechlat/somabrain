"""Transaction Event Store for SomaBrain.
...
VIBE Compliance:
    ...
    - Real PostgreSQL storage (no mocks)
...
class TransactionEventStore:
    ...
    def _store_to_outbox(self, event: TransactionEvent) -> None:
        """Store event to PostgreSQL outbox for durability."""
        try:
    ...
    def query(
        ...
        """Query transaction events from PostgreSQL.
        ...
        try:
            ...
            with session_factory() as session:
                query = session.query(OutboxEvent).filter(
                    OutboxEvent.topic == TRANSACTION_TOPICS["events"]
                )
