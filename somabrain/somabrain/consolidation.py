...
from .metrics import CONSOLIDATION_RUNS, REPLAY_STRENGTH, REM_SYNTHESIZED
...
def _coords_for_payloads(mem: MemoryClient, payloads: List[dict]) -> List[Tuple[float, float, float]]:
...
def run_nrem(tenant_id: str, cfg: Config, mtwm: MultiTenantWM, mtmem: MultiTenantMemory, top_k: int = 16, max_summaries: int = 3) -> dict:
    ...
    payload = {"fact": f"summary: {summary}", "memory_type": "semantic", "phase": "NREM"}
    ...
    try:
        ...
        if hasattr(mem, 'decay_links'):
            pruned = mem.decay_links(factor=float(getattr(cfg, 'link_decay_factor', 0.98) or 0.98), min_weight=float(getattr(cfg, 'link_min_weight', 0.05) or 0.05))
    except Exception:
        pruned = 0
    return {"created": 1, "reinforced": reinforced, "pruned": pruned}


def run_rem(tenant_id: str, cfg: Config, mtwm: MultiTenantWM, mtmem: MultiTenantMemory, recomb_rate: float = 0.2, max_summaries: int = 2) -> dict:
    mem = mtmem.for_namespace(f"{cfg.namespace}:{tenant_id}")
    episodics = _episodics_from_wm(mtwm, tenant_id, limit=256)
    if len(episodics) < 2:
        return {"created": 0}
...
def main():
    ...
    if args.nrem:
        stats = run_nrem(tenant, cfg, mtwm, mtmem, top_k=cfg.nrem_batch_size, max_summaries=cfg.max_summaries_per_cycle)
        print("NREM:", stats)
    if args.rem:
        stats = run_rem(tenant, cfg, mtwm, mtmem, recomb_rate=cfg.rem_recomb_rate, max_summaries=cfg.max_summaries_per_cycle)
        print("REM:", stats)
