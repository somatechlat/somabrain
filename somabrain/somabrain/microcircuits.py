...
from .metrics import MICRO_VOTE_ENTROPY, MICRO_COLUMN_ADMIT, MICRO_COLUMN_BEST
...
class MultiColumnWM:
    ...
    def _ensure(self, tenant_id: str) -> List[WorkingMemory]:
        ...
        if cols is None:
            cols = [WorkingMemory(capacity=self.cfg.per_col_capacity, dim=self.dim) for _ in range(max(1, int(self.cfg.columns)))]
