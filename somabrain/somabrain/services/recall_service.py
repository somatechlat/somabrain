...
def recall_ltm(mem_client, text: str, top_k: int, universe: Optional[str], cohort: str, use_sdr: bool, sdr_enc, sdr_idx_map: dict, graph_hops: int, graph_limit: int) -> Tuple[List[dict], List[Tuple[float, dict]]]:
    ...
    if use_sdr and sdr_enc is not None and hasattr(mem_client, 'all_memories'):
        try:
            idx = sdr_idx_map.setdefault(getattr(mem_client.cfg, 'namespace', 'default'), LSHIndex(bands=sdr_enc.cfg.bands if hasattr(sdr_enc, 'cfg') else 8, rows=sdr_enc.cfg.rows if hasattr(sdr_enc, 'cfg') else 16, dim=sdr_enc.dim if hasattr(sdr_enc, 'dim') else 16384))
    ...
    if universe:
        mem_payloads = [p for p in mem_payloads if str(p.get('universe') or 'real') == str(universe)]
...
def diversify_payloads(embed: Callable[[str], 'np.ndarray'], query: str, payloads: List[dict], method: str = "mmr", k: Optional[int] = None, lam: float = 0.5) -> List[dict]:
    ...
    try:
        ...
        ordered = [payloads[i] for i in selected] + [payloads[i] for i in range(len(payloads)) if i not in selected]
        return ordered
    except Exception:
        return payloads


async def recall_ltm_async(mem_client, text: str, top_k: int, universe: Optional[str], cohort: str, use_sdr: bool, sdr_enc, sdr_idx_map: dict, graph_hops: int, graph_limit: int) -> Tuple[List[dict], List[Tuple[float, dict]]]:
    # Reuse sync SDR prefilter; then prefer async HTTP recall if available
    payloads, hits = recall_ltm(mem_client, text, top_k, universe, cohort, use_sdr, sdr_enc, sdr_idx_map, graph_hops, graph_limit)
    if payloads:
