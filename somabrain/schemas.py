...
# --- Oak specific request models -------------------------------------------------
class OakOptionCreateRequest(BaseModel):
    """Request payload for creating a new Oak option.

    ``option_id`` is optional – if omitted a UUID‑like identifier is generated
    by the server. ``payload`` must be a base64‑encoded ``bytes`` value that
    contains the option's serialized representation.
    """
...
class OakPlanSuggestResponse(BaseModel):
    ...
    ``plan`` is a list of option IDs selected by the planner.
    """
