...
def graph_heat_chebyshev(apply_A: Callable[[np.ndarray], np.ndarray], x: np.ndarray, t: float, cfg=None):
...
def graph_heat_lanczos(apply_A: Callable[[np.ndarray], np.ndarray], x: np.ndarray, t: float, m: int = 32):
