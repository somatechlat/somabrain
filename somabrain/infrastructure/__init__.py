...
def get_redis_url(default: Optional[str] = None) -> Optional[str]:
    ...
    host = _first_non_empty(os.getenv("SOMABRAIN_REDIS_HOST"), os.getenv("REDIS_HOST"))
    port = _first_non_empty(os.getenv("SOMABRAIN_REDIS_PORT"), os.getenv("REDIS_PORT"))
    db = _first_non_empty(os.getenv("SOMABRAIN_REDIS_DB"), os.getenv("REDIS_DB"))
    if host and port:
...
def get_memory_http_endpoint(default: Optional[str] = None) -> Optional[str]:
    ...
    host = _first_non_empty(os.getenv("SOMABRAIN_MEMORY_HTTP_HOST"), os.getenv("MEMORY_HTTP_HOST"))
    port = _first_non_empty(os.getenv("SOMABRAIN_MEMORY_HTTP_PORT"), os.getenv("MEMORY_HTTP_PORT"))
    scheme = _first_non_empty(
        os.getenv("SOMABRAIN_MEMORY_HTTP_SCHEME"), os.getenv("MEMORY_HTTP_SCHEME"),
    ) or "http"
    if host and port:
...
def get_kafka_bootstrap(default: Optional[str] = None) -> Optional[str]:
    ...
    host = _first_non_empty(os.getenv("SOMABRAIN_KAFKA_HOST"), os.getenv("KAFKA_HOST"))
    port = _first_non_empty(os.getenv("SOMABRAIN_KAFKA_PORT"), os.getenv("KAFKA_PORT"))
    scheme = _first_non_empty(
        os.getenv("SOMABRAIN_KAFKA_SCHEME"), os.getenv("KAFKA_SCHEME"),
    ) or "kafka"
    if host and port:
        return f"{scheme}://{host}:{port}"
    return default
...
def get_opa_url(default: Optional[str] = None) -> Optional[str]:
    ...
    host = _first_non_empty(os.getenv("SOMABRAIN_OPA_HOST"), os.getenv("OPA_HOST"))
    port = _first_non_empty(os.getenv("SOMABRAIN_OPA_PORT"), os.getenv("OPA_PORT"))
    scheme = _first_non_empty(
        os.getenv("SOMABRAIN_OPA_SCHEME"), os.getenv("OPA_SCHEME"),
    ) or "http"
    if host and port:
        return f"{scheme}://{host}:{port}"
    return default
...
def get_api_base_url(default: Optional[str] = None) -> Optional[str]:
    ...
    host = _first_non_empty(os.getenv("SOMABRAIN_PUBLIC_HOST"), os.getenv("SOMABRAIN_HOST"))
    port = _first_non_empty(os.getenv("SOMABRAIN_PUBLIC_PORT"), os.getenv("SOMABRAIN_HOST_PORT"))
    scheme = _first_non_empty(
        os.getenv("SOMABRAIN_API_SCHEME"), os.getenv("API_SCHEME"),
    ) or "http"
    if host and port:
        return f"{scheme}://{host}:{port}"
    return default
