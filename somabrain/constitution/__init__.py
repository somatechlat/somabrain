...
class ConstitutionEngine:
    ...
    def _load_pubkey_map(self) -> Dict[str, str]:
        ...
        vault_path = getattr(settings, "SOMABRAIN_VAULT_PUBKEY_PATH", None)
        ...
        # Use env/PATH
        if not mapping:
            env_value = getattr(settings, "SOMABRAIN_CONSTITUTION_PUBKEYS", None)
            single = getattr(settings, "SOMABRAIN_CONSTITUTION_PUBKEY_PATH", None)
            if env_value:
                try:
