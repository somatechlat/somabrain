...
| Variable | Default | Purpose |
|----------|---------|---------|
| `OPA_POLICY_DIR` | `/policies` | Directory containing Rego policies (see `services/opa_sidecar/`). |
| `OPA_ADDR` | `0.0.0.0:8181` | Bind address (set via Helm chart). |

Policies are versioned under `services/opa_sidecar/policies`. The default `leader_policy.rego` prefers the state stream unless entropy exceeds the configured threshold.

---

## Configuration Sources & Priority

1. **Helm values** (`infra/helm/charts/soma-apps/values.yaml`) – declare defaults for every environment. The `envCommon` block maps directly to the tables above.
...
- Toggle the entire cognitive-thread pipeline by setting `featureFlags.enableCogThreads` in Helm values or exporting `ENABLE_COG_THREADS=1` across the involved deployments.
