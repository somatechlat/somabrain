...
- Publishing and leader election run whenever cognitive threads are enabled (default on).

## Settings (single source: `common.config.settings` / env)
- `ENABLE_COG_THREADS` (default `1`)
...
## Operational Notes
- Cognitive threads are enabled by default; only flip `ENABLE_COG_THREADS=0` for emergency gating.
