...
It ships as a **Django Ninja** service with a documented REST surface, BHDC hyperdimensional computing under the hood, and a full Docker stack (Redis, Kafka, OPA, Postgres, Prometheus) so you can test the whole brain locally.
...
| **Complete Docs** | Four-manual documentation suite (User, Technical, Development, Onboarding) plus a change log (`docs/reference/changelog.md`). |
...
HTTP Client
   ...
   ▼
Django Ninja Runtime (somabrain/api/v1.py)
   │   ├─ Authentication & tenancy guards (Django Middleware/Ninja Auth)
...
Docker Compose (`docker-compose.yml`) starts the API plus Redis, Kafka, OPA, Postgres, Prometheus, and exporters. The API exposes container port 9696; Compose maps host port `30101` by default (override with `SOMABRAIN_HOST_PORT`):
...
```bash
$ curl -s http://localhost:30101/memory/remember \
    -H "Content-Type: application/json" \
    -d '{"payload": {"task": "kb.paris", "content": "Paris is the capital of France."}}'

$ curl -s http://localhost:30101/memory/recall \
    -H "Content-Type: application/json" \
...
```bash
$ curl -s http://localhost:30101/context/feedback \
    -H "Content-Type: application/json" \
    -d '`
...
```bash
$ curl -s http://localhost:30101/context/adaptation/state | jq
```

Metrics are available at `http://localhost:30101/metrics`; adaptation gains/bounds under `somabrain_learning_gain` / `somabrain_learning_bound`.

---

## Monitoring (no dashboards)

Prometheus metrics and Alertmanager alerts only. See `docs/technical/monitoring.md`.
...
curl -fsS http://127.0.0.1:30101/health | jq .
curl -fsS http://127.0.0.1:30101/metrics | head -n 20
```

For common queries, see the PromQL cheat sheet at the top of the monitoring guide.

Operational playbooks and escalation examples: see `docs/operations/playbooks/`.
...
See Technical Manual > Predictors (`docs/technical/predictors.md`) for math, config, and tests.
...
- docs/technical/benchmarks-quickstart.md

## Documentation

- **User Manual** – Installation, quick start, feature guides, FAQ (`docs/user/`).
- **Technical Manual** – Architecture, deployment, monitoring, runbooks, security (`docs/technical/`).
- **Development Manual** – Repository layout, coding standards, testing strategy, contribution workflow (`docs/development/`).
- **Onboarding Manual** – Project context, code walkthroughs, checklists (`docs/onboarding/`).
- **Change Log** – Release history and hardening notes (`docs/reference/changelog.md`).
- **Cognitive Threads (Ops)** – Predictor/integrator configuration and health (`docs/technical/runbooks/cog-threads-operations.md`).
- **Predictor Service API** – Predictor math, config, and emitted schema (`docs/technical/predictors.md`).
...
- **Tau & Entropy Alignment** – See `docs/technical/tau-entropy-alignment.md` for how tau annealing, context diversity heuristic, and entropy caps co-exist.

---

## Contributing & Next Steps

1. Read the [Development Manual](docs/development/index.md) and follow the local setup + testing instructions (`pytest`, `ruff`, `mypy`).
...
4. File issues or update `docs/reference/changelog.md` whenever you add a new capability or harden an assumption.
