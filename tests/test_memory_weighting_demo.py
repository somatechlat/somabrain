...
def test_weighting_effect(enable_weighting, client, monkeypatch):
    ...
    if enable_weighting:
        ...
        monkeypatch.setenv("SOMABRAIN_MEMORY_PHASE_PRIORS", "bootstrap:1.1,general:1.0,specialized:1.05")
        monkeypatch.setenv("SOMABRAIN_MEMORY_QUALITY_EXP", "1.0")

    # Insert three memories with similar text so base cosine is close
    client.remember("m1", {"content": "alpha beta gamma", "phase": "general", "quality_score": 0.4})
    client.remember("m2", {"content": "alpha beta gamma", "phase": "bootstrap", "quality_score": 0.9})
    client.remember("m3", {"content": "alpha beta gamma", "phase": "specialized", "quality_score": 0.6})
