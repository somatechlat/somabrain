...
learning_rate_strategy = st.floats(min_value=0.001, max_value=0.5, allow_nan=False, allow_infinity=False)
    ...
    **Validates: Requirements 4.3**
    ...
    def test_exponential_anneal_decreases(
        ...
        assert result <= initial_tau, (
            f"Annealed tau {result} > initial {initial_tau}"
        )
    ...
    def test_exponential_anneal_respects_floor(
        ...
        assert result >= floor, (
            f"Annealed tau {result} < floor {floor}"
        )
...
class TestAdaptationReset:
    ...
    **Validates: Requirements 4.6**
    ...
    @given(
        modified_alpha=st.floats(min_value=2.0, max_value=5.0, allow_nan=False, allow_infinity=False),
        modified_gamma=st.floats(min_value=0.5, max_value=0.9, allow_nan=False, allow_infinity=False),
    ...
    @given(
        default_alpha=st.floats(min_value=0.5, max_value=2.0, allow_nan=False, allow_infinity=False),
    ...
    @given(
        initial_alpha=st.floats(min_value=1.0, max_value=3.0, allow_nan=False, allow_infinity=False),
    )
    @hyp_settings(max_examples=100, deadline=5000)
