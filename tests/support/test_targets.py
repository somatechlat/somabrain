...
DEFAULT_API_URL = _settings.SOMABRAIN_API_URL
DEFAULT_MEMORY_HTTP_ENDPOINT = _settings.SOMABRAIN_MEMORY_HTTP_ENDPOINT
...
def _default_target() -> TargetConfig:
    ...
    return TargetConfig(
        ...
        redis_url=redis_url,
        tenant=settings.SOMABRAIN_DEFAULT_TENANT,
        postgres_dsn=settings.SOMABRAIN_POSTGRES_DSN,
        bypass_lock_checks=_env_truthy(settings.SOMABRAIN_API_URL_lock_bypass),
    )
...
def _live_target_from_env() -> TargetConfig | None:
    ...
    tenant = settings.SOMABRAIN_LIVE_TENANT
    postgres = settings.SOMABRAIN_LIVE_POSTGRES_DSN
    bypass = _env_truthy(settings.SOMABRAIN_LIVE_FORCE)
    return TargetConfig(
        label=settings.SOMABRAIN_LIVE_LABEL or "live",
        api_base=api,
        memory_base=memory,
        redis_url=redis_url,
        tenant=tenant,
