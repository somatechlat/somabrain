...
def test_spectral_heat_diffusion(high_dim_quantum):
    ...
    L = np.eye(high_dim_quantum.cfg.dim) - np.roll(
        np.eye(high_dim_quantum.cfg.dim), 1, axis=1
    )
    L = L @ L.T
...
def test_density_matrix_entropy(high_dim_quantum):
    ...
    assert entropy < max_entropy, (
        f"Mixed state entropy exceeds maximum: {entropy} > {max_entropy}"
    )


def test_role_frame_properties(high_dim_quantum):
    """Verify properties of the role frame (approximately tight frame)."""
    roles = [high_dim_quantum.make_unitary_role(f"test::role::{i}") for i in range(10)]
