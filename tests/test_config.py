...
SOMABRAIN_API_URL = require(
    get_api_base_url(DEFAULT_API_URL) or os.getenv("SOMA_API_URL") or DEFAULT_API_URL,
...
POSTGRES_DSN = require(
    get_postgres_dsn(DEFAULT_POSTGRES_DSN)
    or os.getenv("SOMABRAIN_POSTGRES_DSN")
...
BACKEND_ENFORCEMENT_MODE = os.getenv(
    "SOMABRAIN_REQUIRE_EXTERNAL_BACKENDS", "1"
).lower() in ("1", "true", "yes")

# Allow fakeredis only for development (never in CI)
ALLOW_FAKEREDIS = os.getenv("SOMABRAIN_ALLOW_FAKEREDIS", "0").lower() in (
    "1",
    "true",
    "yes",
)
...
FORCE_FULL_STACK = os.getenv("SOMABRAIN_FORCE_FULL_STACK", "1").lower() in (
    "1",
    "true",
    "yes",
)
...
REQUIRE_MEMORY = os.getenv("SOMABRAIN_REQUIRE_MEMORY", "1").lower() in (
    "1",
    "true",
    "yes",
)
...
if __name__ == "__main__":
    ...
    for service, url in CONFIG_SUMMARY.items():
        status = (
            "✅ Available"
            if is_service_available(service.split("_")[0])
            else "❌ Unavailable"
        )
