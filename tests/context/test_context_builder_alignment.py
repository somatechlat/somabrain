...
class _DummyMemory:
    ...
    def search_text(self, query_text: str, top_k: int, filters=None):  # pragma: no cover - simple test shim
        res = []
            ...
            res.append(
                {
                    ...
                    "embedding": (np.ones(8) if i % 2 == 0 else np.ones(8) * 0.5).tolist(),
                }
            )
        return res
...
def test_context_builder_entropy_cap_from_overrides(monkeypatch):
    monkeypatch.setenv("SOMABRAIN_LEARNING_TENANTS_OVERRIDES", json.dumps({"public": {"entropy_cap": 1.0}}))
    mem = _DummyMemory(["a", "a", "b", "c"])  # duplicates to exercise tau change but not required
    ...
    H = _entropy_vec(cb.weights.alpha, cb.weights.beta, cb.weights.gamma, cb.weights.tau)
    assert H <= 1.0 + 1e-6
